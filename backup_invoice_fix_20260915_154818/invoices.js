...
import { cancelInvoice, createInvoice, deleteInvoice, duplicateInvoice, getInvoice, listInvoices, reserveInvoiceNumber, updateInvoice } from "../services/invoiceService.js";
...
invoicesRouter.post("/next-number", async (req, res, next) => {
  try { res.json({ success: true, data: await reserveInvoiceNumber() }); } catch (error) { next(error); }
});
